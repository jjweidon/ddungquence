...
### 5. 로비 관전하기 버튼
- [ ] 로비 페이지에 **관전하기** 버튼을 추가한다.
- [ ] 관전자는 **팀 배정에서 제외**되는 것으로 인지해야 한다(UI/문구로 명시).
- [ ] 관전자가 다시 게임에 참여할 때: **4명이 이미 차 있으면 참여 불가**.
- [ ] 참여 가능할 때는 **방 입장 시와 같은 로직**으로 팀이 배정되어야 한다.

### 6. 방 생성자 자동 준비
- [ ] **방을 생성한 사람(host)**은 생성 직후 자동으로 **준비 상태**가 되도록 한다.

### 7. 게임 순서 = 준비 시점 기준(팀 내 순서 조정)
- [ ] 게임 순서는 **플레이어 목록**을 기반으로 한다.
- [ ] 현재는 방 입장 순서로 정해져 있는데, **가장 마지막에 준비 상태가 된 시점**을 기준으로 순서가 정해지도록 변경한다.
- [ ] **레드 → 블루 → 레드 → 블루** 팀 순서는 유지된다.
- [ ] 의도: 팀 내에서 순서가 마음에 안 들면 **준비 상태를 토글**하면서 순서를 변경할 수 있어야 한다.
