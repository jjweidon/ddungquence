...
### 2. 상대 턴에도 손패로 위치 확인
- [ ] **내 턴이 아닐 때도** 손패 카드를 클릭할 수 있다.
- [ ] 클릭한 카드에 대응하는 보드 칸을 하이라이트(또는 표시)하여 위치를 확인할 수 있게 한다.
- [ ] 상대 턴일 때는 **보드 칸(카드)은 클릭 불가** — 선택/하이라이트만 보여주고 액션은 불가.
...
- [ ] **시간 종료 시** 자동 처리:
  ...
  - 해당 카드에 대응하는 **가능한 칸 중 하나**에 칩 배치(빈 칸이 있으면 그중 하나; 두 칸 모두 비어 있으면 하나 선택).
