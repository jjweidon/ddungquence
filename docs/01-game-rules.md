...
**잭 종류 판별**: 카드 ID의 **문양(suit)**으로 구분한다. 번호(j_1/j_2)가 아니다.
- **클로버(clover)·다이아(diamond) 잭** → Two-eyed Jack (와일드 배치), j_1·j_2 모두
- **스페이드(spade)·하트(heart) 잭** → One-eyed Jack (칩 1개 제거, 상대·자기 팀 모두 가능), j_1·j_2 모두

### 5.1 Two-eyed Jack (2-eye) = 와일드 배치
- 클로버 또는 다이아 잭(예: `clover_j_1`, `clover_j_2`, `diamond_j_1`, `diamond_j_2`)을 버리고, 보드의 **어떤 빈 칸**에도 내 칩 1개를 배치 가능.
...
- 스페이드 또는 하트 잭(예: `spade_j_1`, `spade_j_2`, `heart_j_1`, `heart_j_2`)을 버리고, 보드 위 칩 1개를 **제거**한다. 상대 팀 칩뿐 아니라 **자기 팀 칩도** 필요 시 제거할 수 있다(예: 6목 해소).
...
- 2시퀀스 승리 모드:
  - **같은 방향(같은 라인)**: 시퀀스끼리 칩을 공유하면 안 됨. 같은 줄에서 겹치지 않는 5칸만 별도 시퀀스로 인정(예: 한 줄 10칸 전부 채우면 [0-4], [5-9] 2시퀀스).
  - **다른 방향(교차)**: 첫 시퀀스의 한 칸을 두 번째 시퀀스에 재사용 가능(중복 1칸 허용).

### 7-1) 6목(6개 이상 연속) 정책(고정)
- **6·7·8·9목은 시퀀스로 인정하지 않는다.** 같은 팀 칩이 동일 방향으로 6개, 7개, 8개, 9개 연속이면 그 구간의 어떤 5칸도 시퀀스로 세지 않는다.
- **5목**: 1시퀀스. **10목 이상**: 같은 라인에서 겹치지 않는 5칸만 시퀀스로 인정(10칸 → 2시퀀스 등).
- 이미 완성된 시퀀스에 1칸을 더 붙여 같은 라인이 6개가 되면, 새 5칸 윈도우는 기존 시퀀스와 같은 라인에서 겹치므로 신규 시퀀스로 인정되지 않는다.
- **6개 연속 → one-eyed Jack으로 양쪽 끝 중 하나를 제거해 5개만 남기면** → 그 5칸이 시퀀스로 인정된다.
