...
/**
 * Two-eyed Jack (와일드 배치): J♣(clover), J♦(diamond)
 * 보드의 어떤 빈 칸에도 아군 칩 배치 가능
 */
export function isTwoEyedJack(cardId: CardId): boolean {
  return cardId.startsWith("clover_j_") || cardId.startsWith("diamond_j_");
}

/**
 * One-eyed Jack (상대 칩 제거): J♥(heart), J♠(spade)
 * 완성된 시퀀스 칩 제거 불가(변형 규칙)
 */
export function isOneEyedJack(cardId: CardId): boolean {
  return cardId.startsWith("heart_j_") || cardId.startsWith("spade_j_");
}
